...
for f, m in zip(files, file_path_to_model):
    print(f, m)
    # model = run_test(f, m)
    ...
    def reshape_transform(tensor, height=8, width=8):
        ...
        return result
    def apply_gradcam_for_single_picture(model, image_path, transform, is_swin=False):
    # Reads a file using pillow
        rgb_img = PIL.Image.open(image_path)

        # The image can be converted to tensor using
        print(model)
        target_layers =  [model.model.layers[-2].blocks[-1].norm1]#[model.layers[-1].blocks[-1].norm1]
        #print(model.layers[-1].blocks[-1].norm1)
        #model.model.layers[-1].blocks[-1].norm1.register_forward_hook(get_output("BasicBlock"))
        output = model(transform(rgb_img).unsqueeze(0).to('cuda'))

        #input_tensor = otensor['BasicBlock'] # Create an input tensor image for your model..
        #print(input_tensor.shape)
        output = model(transform(rgb_img).unsqueeze(0).to('cuda'))
        
	if is_swin:
		cam = GradCAM(model, target_layers=target_layers, reshape_transform=reshape_transform)
	else:
	cam = GradCAM(model, target_layers=target_layers)

    
        grayscale_cam = cam(input_tensor=transform(rgb_img).unsqueeze(0))
        ...
        return cam_image#, model_outputs
    ...
    print(list_dirs)
    for i in range(6):
        list_dirs[i] = os.listdir(test_path[i])

    for i in range(6):
        for directory in list_dirs[i]:
            path = os.path.join(test_path[i], directory)
            vis = apply_gradcam_for_single_picture(model, path, transform, is_swin)
            save_path = os.path.join("gradcam", test_path[i], directory)
